...
def tff_parse(font_parse_name):
    ...
    for i in font_base_order:  # 遍历 uniE77C 拿到字形的编码
        flags = encrypt_md5(str(list(font_base['glyf'][i].coordinates)))  # x,y坐标属性
        # flags = font_base['glyf'][i].coordinates   # x,y坐标属性
        # f_base_flag.append(list(flags))  # 获取到 flags 值 并写入列表
        f_base_flag.append(flags)  # 获取到 flags 值 并写入列表

    f_flag = []
    for i in font_parse_order:
        flags = encrypt_md5(str(list(font_parse['glyf'][i].coordinates)))
...
def comp(L1, L2):
    if encrypt_md5(str(L1)) == encrypt_md5(str(L2)):
        return 1
    else:
        return 0
...
if __name__ == '__main__':
    ...
    res = replace_content(names = """2020謍06月10糾09蓖00分""")
    print(res)
